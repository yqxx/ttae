...
public class JwtTokenUtil implements Serializable {
	...
	public JwtAuthenticationResponse generateToken(UserDetails userDetails) {
		...
		JwtAuthenticationResponse response = new JwtAuthenticationResponse(generateToken(claims, expirationDate), expiration);
		return response;
	}

	String generateToken(Map<String, Object> claims, Date expiration) {
		return Jwts.builder().setClaims(claims).setExpiration(expiration).signWith(SignatureAlgorithm.HS512, secret)
				.compact();
	}
